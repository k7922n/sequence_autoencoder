...
class Seq2SeqModel(object):
  ...
  def __init__(self,
    ...
    Args:
      vocab_size: size of the whole vocabulary.
    ...
    if forward_only:
      self.outputs, self.losses, self.states = my_seq2seq.model_with_buckets(
  ...
  def get_batch(self, data, bucket_id):
    ...
    for _ in xrange(self.batch_size):
      ...
      encoder_pad = [my_data_util.PAD_ID] * (encoder_size - len(encoder_input))
      encoder_inputs.append(list(reversed(encoder_input + encoder_pad)))

      # Decoder inputs get an extra "GO" symbol, and are padded then.
      decoder_pad_size = decoder_size - len(decoder_input) - 1
      decoder_inputs.append([my_data_util.GO_ID] + decoder_input +
                            [my_data_util.PAD_ID] * decoder_pad_size)
    ...
    for length_idx in xrange(decoder_size):
      ...
      for batch_idx in xrange(self.batch_size):
        ...
        if length_idx == decoder_size - 1 or target == my_data_util.PAD_ID:
